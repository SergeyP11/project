...
#include <algorithm> // STL (сортировка, поиск)
#include <random> // генерация случайных чисел
#include <set> // использование контейнера множество (set)
#include <utility> // утилитарные функциями (std::move)
...
{
public:
    struct Cell // Структура, представляющая узел стека
    {
        T value; // Значение, хранимое в узле
        Cell *next; // Указатель на следующий узел
    };
    Cell *beginning = nullptr; // Указатель на верхний элемент стека
    void output(Cell *a) // Рекурсивный метод для вывода элементов стека
    {
...
public:
    ...
    {
        Cell *c = new Cell(); // Создаем новый узел
    ...
    T pop() // Метод для удаления элемента из стека
    {
        ...
        {
            Cell *a = beginning; // Временный указатель на верхний элемент
    ...
    {
    private:
        Cell* current; // Указатель на текущий элемент
        Cell* prev; // Указатель на предыдущий элемент
    public:
        Iterator(Cell* cell) : current(cell) {} // Конструктор итератора
        Iterator& operator++() // Оператор инкремента для перехода к следующему элементу
        {
...
public:
    ...
    Edition(int all, int sold) : amount_of_all_tickets(all), amount_of_sold_tickets(sold) {} // Конструктор для инициализации полей
};
...
template <typename T>
void generate_the_tickets(std::mt19937 g, int amount_of_tickets, T& tickets, int amount_of_prizes_types, ...)
{
    if (amount_of_tickets <= 0 || amount_of_prizes_types <= 0 )
    {
        throw std::invalid_argument("invalid argument in generate_the_tickets()");
    }
    std::vector<int> prize_money, prize_type_amount_of_tickets; // векторы для хранения призовых сумм и количества билетов каждого типа
    va_list args;
    ...
    for (int i = 0; i < amount_of_prizes_types; ++i)
    {
        ...
        prize_type_amount_of_tickets.push_back(a); // Добавление количества билетов в вектор
        int hold = va_arg(args, int); // Извлечение следующего аргумента, представляющего призовую сумму
        for (auto i = 0; i < a; ++i)
        {
            prize_money.push_back(hold); // Добавление призовой суммы в вектор соответствующее количество раз
        }
    }
    va_end(args);
    ...
    {
        sh_numbers.push_back(i); // Заполнение вектора номерами билетов от 1 до amount_of_tickets
    }
    ...
    std::set<int> uniqueNumbers; // Множество для хранения уникальных номеров билетов с призами
    while (uniqueNumbers.size() < amount_of_prize_tickets)
    {
        std::uniform_int_distribution<int> distribution(1, amount_of_tickets); // Создание распределения для случайного выбора номера билета
        int random = distribution(g); // Генерация случайного номера билета
        uniqueNumbers.insert(random); // Добавление случайного номера в множество (если номер уже существует, он не будет добавлен)
    }
    int counter = 0; // Счетчик для призовых сумм
    for (auto i = 1; i <= amount_of_tickets; ++i)
    {
        auto it = uniqueNumbers.find(sh_numbers[i-1]); // Проверка, есть ли текущий номер билета в множестве призовых номеров
        if (it != uniqueNumbers.end())
        {
            Ticket ticket(sh_numbers[i-1], prize_money[counter]); // Создание билета с призом
...
int main()
{
    try
    {
        ...
        int amount_of_tickets = 25000000; // Общее количество билетов
        ...
        while (true) {
            ...
            switch (option) {
                case 1: {
                    MyStack<Ticket> tickets1;
                    MyStack<Ticket> winners1;
                    Edition<MyStack<Ticket>> edition1(amount_of_tickets, sold);
                    generate_the_tickets(g, edition1.amount_of_all_tickets, tickets1, 4, 1, 1000000, 2, 750000, 3, 500000, 4, 250000);
                    calculate_the_winners(winners1, edition1.amount_of_sold_tickets, tickets1);
                    for (auto i = winners1.begin(); i != winners1.end(); ++i) {
                        std::cout << "The number of the winning ticket:" << (*i).num_prize.first << ", the prize is " << (*i).num_prize.second << " rubles" << std::endl;
                    }
                    break;
                }
                case 2: {
                    MyForwardList<Ticket> tickets2;
                    MyForwardList<Ticket> winners2;
                    Edition<MyForwardList<Ticket>> edition2(amount_of_tickets, sold);
                    generate_the_tickets(g, edition2.amount_of_all_tickets, tickets2, 4, 1, 1000000, 2, 750000, 3, 500000, 4, 250000);
                ...
                case 3: {
                    MyQueue<Ticket> tickets3;
                    MyQueue<Ticket> winners3;
                    Edition<MyQueue<Ticket>> edition3(amount_of_tickets, sold);
                    generate_the_tickets(g, edition3.amount_of_all_tickets, tickets3, 4, 1, 1000000, 2, 750000, 3, 500000, 4, 250000);
                ...
                case 4: {
                    MyVector<Ticket> tickets4;
                    MyVector<Ticket> winners4;
                    Edition<MyVector<Ticket>> edition4(amount_of_tickets, sold);
                    generate_the_tickets(g, edition4.amount_of_all_tickets, tickets4, 4, 1, 1000000, 2, 750000, 3, 500000, 4, 250000);
